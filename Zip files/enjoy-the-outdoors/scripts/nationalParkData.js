...
// function to put all the states and territories into the dropdown menu
function populateStatesDropdown() {
    const stateDropdown = document.getElementById("park-state");

    // basically makes a new array of just the states + territories
...
function filterParks() {
    ...
    // filteredParks will equal the filtered array 
    const filteredParks = nationalParksArray.filter(park =>
        // selectedType has to equal blank OR the park type has to INCLUDE the selected type
        (selectedType === "" || park.LocationName.includes(selectedType)) &&

        // AND selectedState has to also equal blank OR the park state has to equal the selected state
        (selectedState === "" || park.State === selectedState)
    );
    // this finally runs the function displayParks with the filteredParks as the parameter
    displayParks(filteredParks);
}

// function that displays the parks in the DOM
function displayParks(parks) {
    const parkInfoContainer = document.getElementById("park-info-container");
    parkInfoContainer.innerHTML = "";

    // for each park in the array, makes this div and puts in all that info
    parks.forEach(park => {
        ...
        parkInfo.innerHTML = `
            ...
            <hr>
        `;
        // finally adds all that info into the DOM 
...
function populateTypeDropdown() {
    // this line gets the park-type dropdown element
    const parkTypeDropdown = document.getElementById("park-type");

    // injects the array into this const 
    const parkTypes = parkTypesArray;
...
// Call the function to populate the park type dropdown
